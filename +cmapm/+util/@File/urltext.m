...
ut = cmapm.util.File.URIType(URI);
...
switch(ut)
    case 'file'
        ...
        status = 1;
    case {'http'}
        [outText, status] = urlread(cmap.util.File.encodeURL(URI));
    case {'fileuri'}
        [outText, status] = urlread(URI);
    case 's3'
        disp('S3 STUB');
        %[outText, status] = cmapm.containers.S3.read(URI);
    otherwise
        error('urltext:InvalidURI', 'Invalid URI %s', ut);
end
