classdef ArgParse < handle
    ...
    % For usage see:
    % help cmapm.common.ArgParse.getArgs
    %
    % Alternatively you can use the class directly as follows:
    %
    % p = cmapm.common.Argparse('myprogram', 'does something';
    %
    ...
    properties (Constant=true, GetAccess=private)
        ...
        % included configs
        config_files = cmapm.containers.Dict();
    ...
    methods (Static=true)
        ...
        [args, help_flag] = getArgs(configFile, opt, varargin);
    ...
    methods
        function obj = ArgParse(prog, desc)
            ...
            %obj.name_ = cmapm.containers.Dict();
            
            % Default arguments            
            name = {{'--help', '-h'}, '--undef_action'};
            default = {'', ''};
            action = {'help', 'undef'};
            help = {'Show this help message and exit', 'Action for undefined argument'};
            default_arg = struct('name', name, 'default', default, 'action', action, 'help', help);
            obj.add(default_arg);
        end
        ...
        function addFile(obj, cfg_file)
            ...
            try
                cfgText = cmapm.util.File.urltext(cfg_file);
            catch me
                disp(me.message)
        ...
        function add(obj, varargin)
            ...
            if ~isempty(input)
                ...
                if ~isempty(input.choices)
                    if cmapm.util.DataType.isNumericType(input.type)
...
%                 input.name = sort(input.name);
                if isa(obj.name_, 'cmapm.containers.Dict') 
        ...
        function args = parse(obj, varargin)
...
%             flag_action = {'store_const', 'store_true', 'store_false', 'help'};
            ...
            for ii=1:obj.nargs
                this_arg = cmapm.util.String.printDelimitedLine(obj.arg_(ii).name, 'dlm', ', ');
                if isequal(obj.arg_(ii).access, 'public')
                    ...
                    if ~isempty(this_default)
                        this_help = sprintf('%s. Default is %s',...
                            this_help,...
                            cmapm.util.String.stringify(this_default));
                    end
                    if ~isempty(this_choices)
                        this_help = sprintf('%s. Options are {%s}',...
                            this_help,...
                            cmapm.util.String.stringify(this_choices));
                    end
            ...
            if ~isempty(obj.info_.description)
                ...
                desc = obj.formatLines_(desc_str, 80);
                fprintf(1, '\nDescription:\n');
                fprintf(1, '%s\n', desc{:});
            end
        end
    ...
    methods(Access = private)
        ...
        function res = parse_(obj, varargin)
            % parse the parameter values.

            narg = length(varargin);
            % check if help is set
            if any(strcmp('--help', varargin) | strcmp('-h', varargin))
                obj.printHelp();
                ME = MException('cmapm:common:ArgParse:HelpRequest',...
                    'Help requested');
                throw(ME);
            end
            ...
            npos = length(pos_arg);
            if(narg<npos)
                ME = MException('cmapm:common:ArgParse:InsufficientArguments',...
        ...
        function undefAction_(obj, param)
            ...
            switch(obj.undef_action)
                case 'warn'
                    warning('cmapm:common:ArgParse:UnknownParameter',...
                        'Unknown parameter %s', param);
                case 'error'
                    error('cmapm:common:ArgParse:UnknownParameter',...
                        'Unknown parameter %s', param);
                case 'ignore'
            end
        end
        ...
        function res = readArgs_(obj, cfg_file, res)
            ...
            cfg_text = cmapm.util.File.urltext(cfg_file);
        ...
        function res = assignValue_(obj, param, val)
            ...
            if isa(val, tgt_type)
                ...
                switch tgt_type
                    ...
                    case {'logical'}
                        ...
                        if ~is_scalar
                            error('cmapm:common:ArgParse:InvalidLogical',...
                        ...
                        elseif ischar(val)
                            ...
                            error('cmapm:common:ArgParse:InvalidLogical', 'Cannot convert %s to logical',...
                                    class(val));
                        end
                    case 'cell'
                        if ischar(val)
                            val = cmapm.util.String.tokenize(val, ',', true);
                        elseif ~iscell(val)
                            error('cmapm:common:ArgParse:InvalidCell',...
            ...
            if ~isempty(cfg.choices)
                ...
                if ~tf
                    error('cmapm:common:ArgParse:InvalidChoice',...
...
%             end
        ...
        function fmt = formatLines_(~, s, width)
            % Formats strings for printing
            s = cmapm.util.String.tokenize(s, char(10));
            nlines = length(s);
            fmt = cell(2*nlines - 1, 1);
            fmt(1:2:end) = s;
            fmt(2:2:end) = {''};
            fmt = strcat({char(9)}, textwrap(fmt, width));
        end
