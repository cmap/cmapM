...
if ~isempty(args.chunk_size)
    ...
    chunk_size = get_chunk_size(class(fill_value), dims, args.max_chunk_kb);
end
...
function chunk_size = get_chunk_size(memtype, dims, max_chunk_kb)
    % Auto chunk size
    switch(memtype)
        case 'H5T_FORTRAN_S1'
            elem_per_kb = 1024;
        case 'H5T_IEEE_F32LE'
            ...
            elem_per_kb = 256;
        otherwise
            error('Unknown memtype: %s', memtype);
    end
...
if ~isempty(args.chunk_size)
    ...
    chunk_size = get_chunk_size('H5T_IEEE_F32LE', dims, args.max_chunk_kb);
end
dcpl = set_compression(args.compression, args.compression_level, chunk_size);
...
chunk_size = get_chunk_size(h5param.memtype, dims, args.max_chunk_kb);
