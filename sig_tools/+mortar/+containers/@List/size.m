...
%   the number of elements in the list A. The second dimension is always 1.
%
%   [NROW, NCOL] = SIZE(A) returns the number of elements in the list A and
%   number of columns (always 1)as separate output variables.
%
...
if nargin == 1
    ...
    if nargout > 1
        error('mortar:containers:dict:size:TooManyOutputs', ...
              'Too many outputs.');
    elseif ~isscalar(dim) || (dim < 1 || 2^31 < dim) || (round(dim) ~= dim)
        error('mortar:containers:dict:size:InvalidDim', ...
