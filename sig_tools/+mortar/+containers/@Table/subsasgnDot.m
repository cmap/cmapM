...
if isnumeric(varName) && isscalar(varName)
    ...
    isNewVar = (varIndex > a.ncols);
    if isNewVar
        varName = newNames{1};
    else
        varName = a.columns{varIndex};
    end
elseif ischar(varName) && size(varName,1) == 1
    varIndex = find(strcmp(varName,a.columns));
    ...
    if isNewVar
        ...
        internal.matlab.codetools.genvalidnames({varName},false); % error if invalid
...
if builtin('_isEmptySqrBrktLiteral',b) && (isscalar(s) || isequal(s(2).type,'()'))
    ...
    % first index.
        ...
        var_j = a.data_{varIndex};
        try %#ok<ALIGN>
            var_j(s(2).subs{:}) = [];
        catch ME, throw(ME); end
        a.data_{varIndex} = var_j;
    end
    ...
    if varLen < a.nrows
        ...
        var_j = lengthenVar(var_j,a.nrows);
    end
    a.data_{varIndex} = var_j;
