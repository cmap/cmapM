...
function h= vertrasterplot(times,numtrials,triallen, varargin)
...
switch nin
 ...
 % plot spikes
  ...
  axes(hresp);
  h=plot(yy, xx, plotcolor, 'linewidth',plotwidth);
  axis ([0,(numtrials)*1.5, xlim]);  
  
  if (showtimescale)
     set(hresp, 'xtick', [],'tickdir','out');        
  else
