function deregister_(obj, varargin)
% deregister_ Remove tool from inventory
...
if nin>=2
    ...
    if nTools
        obj.printList_;
        reply = getInput(sprintf('Enter index of Sig Tool to deregister [1-%d]:', nTools),...
...
if ~isempty(opt.ToolName)
    ...
    if isfield(prop, 'sig_tool_list')
        prop.sig_tool_list = setdiff(prop.sig_tool_list, opt.ToolName,...
            'stable');
    else
