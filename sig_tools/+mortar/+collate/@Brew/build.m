...
if ~help_flag
    ...
    for ii=1:nb
        ...
        has_rec_field = all(isfield(annot_sig, zs_fields));
        if isfileexist(has_rec_field)
            Message.debug(args.verbose, 'Collating gposcon results from : %s', zstats_well_file);
            recall_sets = parse_record(zstats_well_file);
            recall_sets = mvfield(recall_sets, {'zs', 'rank'}, zs_fields);
            annot_sig = join_table(annot_sig, recall_sets, 'sig_id', 'sample_id',...
                zs_fields);
        elseif has_rec_field
            warning('ZSTATS_WELL file not found, skipping: %s', zstats_well_file);
        end
    end
    %% collate recall stats if available
    recall_fields = {'median_recall_score', 'median_recall_rank'};
    recall_metrics = {'spearman', 'wtcs.lm50'};
    %% check for recall reports for both wtcs and spearman
    %% and use whichever exist
    for ii=1:nb
        this_brew = brew_list{ii};
        for jj=1:length(recall_metrics)
            this_metric = recall_metrics{jj};
            this_recall_file = fullfile(args.brew_path, brew_list{ii}, ...
                fullfile(brew_location, 'recall', this_metric, this_brew, 'recall_report_sets.txt'));
            has_rec_field = all(isfield(annot_sig, recall_fields));
            if isfileexist(this_recall_file)
                Message.debug(args.verbose, 'Collating recall results from : %s', this_recall_file);
                recall_sets = parse_record(this_recall_file);
                new_recall_fields = strrep(strcat(recall_fields, '_', this_metric), '.', '_');
                recall_sets = mvfield(recall_sets, recall_fields, new_recall_fields);
                annot_sig = join_table(annot_sig, recall_sets, 'sig_id', 'recall_group',...
                    new_recall_fields, -666);
            elseif has_rec_field
                warning('RECALL file not found, skipping: %s', this_recall_file);
            end
        end
    end
    %% Ranks
