function [h, recall_summary] = plotPerPair(rpt, varargin)
% plotPerPair Generate diagnostic plots for recall analysis
...
xlim([0, 100]);
title(sprintf('n=%d', npoints));
xlabel(sprintf('Rank (max=%d)', max_rank));
if ~isempty(args.ylabelrt)
    ylabelrt(texify(args.ylabelrt), 'color', 'b');
end
namefig('cdf_rank');
