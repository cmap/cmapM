...
for ii=1:nanalyte    
    ...
    if ismember(analyte(ii), arg.notduo)
        pkstats(analyte(ii)) = mortar.compute.L1kPipeline.detect_lxb_peaks_single(rp1(rid==analyte(ii)), 'ksmethod', 'median');
    else
        try
        pkstats(analyte(ii)) = mortar.compute.L1kPipeline.detect_lxb_peaks_single(rp1(rid==analyte(ii)), varargin{:});
        catch e
            disp(e);
