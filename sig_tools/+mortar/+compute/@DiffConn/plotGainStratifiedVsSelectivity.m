function [row_meta, h] = plotGainStratifiedVsSelectivity(ds, sel_rpt)
...
ds_filt = mortar.compute.DiffConn.filterBestConnections(ds, 85, 2);
% stratified gain
...
scatter(sel_binned(~is_sig_score) + randn(size(sel_binned(~is_sig_score))), [row_meta(~is_sig_score).gain_strat]', 6, cnx_score(~is_sig_score), 'filled')
hold on
scatter(sel_binned(is_sig_score) + randn(size(sel_binned(is_sig_score))), [row_meta(is_sig_score).gain_strat]', 9, cnx_score(is_sig_score), 'filled')
colorbar
caxis([-100, 100]);
colormap(taumap_redblue85)
xlabel('Selectivity Percentiles')
ylabel('DiffConn Score (Stratified)')
title(sprintf('%s, %d features %d perts %d signif', query_name, num_features, num_perts, num_signif))
namefig(sprintf('%s_sel_vs_gain_strat', query_name));
end
