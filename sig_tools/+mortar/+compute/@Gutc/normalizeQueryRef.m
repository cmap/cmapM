function [ncs, rpt, pos_mean, neg_mean] = normalizeQueryRef(cs, rid_ref, varargin)
...
% asssume ctl_vehicle is trt_cp
cs_type = strrep(cs_type, 'ctl_vehicle', 'trt_cp');
% TODO: handle ctl_vehicle

cs_cell = cs.rdesc(:, cs.rdict('cell_id'));
csgp = strcat(cs_type,':',cs_cell);

[cmn_rid, ridx_ref, ridx_cs] = intersect(rid_ref, cs.rid, 'stable');
assert(isequal(numel(cmn_rid), numel(rid_ref)),...
        'Some reference signatures missing from query dataset')
is_rid_ref = ismember(cs.rid, rid_ref);
...
gp_type = cs_type(uidx);
gp_cell = cs_cell(uidx);
...
% [gpvar, gpn, gpidx]=get_groupvar(cs.rdesc, cs.rhd, {'pert_type', 'cell_id'});

% ngp = length(gpn);
% useme = find(~isnan(gpidx));
% [~, uidx] = unique(gpidx(useme));
% tsgpn = gpvar(useme(uidx));
% ntsgp = length(tsgpn);
