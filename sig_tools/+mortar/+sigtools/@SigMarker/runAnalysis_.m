...
res.args = args;
% read inputs
[ds, cl, cl_discarded] = getInput(args);
res.cl_discarded = cl_discarded;
...
for ii=1:nsig
    ...
    if ii>1
        score_ds = merge_two(score_ds, this_score);
...
nmarker = clip(args.nmarker, 1, length(score));
dbg(1, 'Computing sig. strength using %d features', nmarker);

% signature strength
ss = mean(score(1:nmarker)) - mean(score(end-nmarker+1:end));
...
switch(lower(args.metric))
    ...
    case 'fc'
        [score_ds, column_order] = computeFC(ds, cl, 'fc');
    otherwise
...
function [ds, cl, cl_discarded] = getInput(args)
...
if ~isempty(args.feature_id)
    feature_id = parse_grp(args.feature_id);
else
    if ~strcmpi('all', args.feature_space)
        assert(mortar.common.Spaces.probe_space.isKey(args.feature_space),...
            'Feature space %s is unknown', args.feature_space); 
        feature_id = mortar.common.Spaces.probe(args.feature_space).asCell;
    else
        feature_id = [];
    end
end
ds = parse_gctx(args.ds, 'cid', cid);
ds = ds_slice(ds, 'rid', feature_id, 'ignore_missing', args.ignore_missing_features);
if ~isempty(args.col_meta)
    ds = annotate_ds(ds, args.col_meta);
end
if ~isempty(args.row_meta)
   ds = annotate_ds(ds, args.row_meta, 'dim', 'row');
end

% remove phenotypes with missing samples
has_missing_cid = ~ismember({cl.sample_id}, ds.cid);
nmissing = nnz(has_missing_cid);
if nmissing
    % discard 
    missing_sig = unique({cl(has_missing_cid).sig_id}');
    to_discard = ismember({cl.sig_id}, missing_sig);
    cl_discarded = cl(to_discard);
    cl(to_discard) = [];
    dbg(1, 'Missing samples for %d signatures, ignoring them', ...
        nnz(to_discard));    
else
    cl_discarded = [];
end
...
function [score_ds, cid_order] = computeFC(ds, cl, metric)
% compteFC compute fold-change statistic.
% Note: Assumes ds.cid and cl.cid are in the same order
...
rnk = rankorder(lfc, 'direc', 'descend', 'fixties', false);
score_ds = mkgctstruct(lfc, 'rid', ds.rid, 'rhd', ds.rhd,...
    'rdesc', ds.rdesc, 'cid', {cl(1).sig_id});
metric_str = metric;
