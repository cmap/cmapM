...
%       'row_filter' Filter(s) specifying rows to extract
%       'column_filter' Filter(s) specifying columns to extract
%       'ignore_missing' Ignore missing ids if true. Default is false.
%       'row_field' Alternate row metadata field to match
%       'column_field' Alternate column metadata field to match
%       'checkid'   Check if row and column ids are unique
...
config = struct('name', {'--rid';'--ridx';...
    ...
    'Filter(s) specifying rows to extract'; 'Filter(s) specifying columns to extract';...
