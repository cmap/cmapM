...
out_file = 'inchified_smiles.txt' if args.out_file is None else args.out_file

# Executed when -smi is provided
if args.smiles is not None:
    res = s2fp.inchify_smiles(args.smiles)
    dataJ = [{'canonical_smiles': args.smiles,
              'inchified_smiles': res.get('inchified_smiles'),
              'inchi': res.get('inchi'),
...
# Executed when -icsv file is provided
    ...
    for i in range(len(dataJ)):
        sm = dataJ[i]['canonical_smiles']
        res = s2fp.inchify_smiles(sm)        
        dataJ[i].update({'inchified_smiles' : res.get('inchified_smiles'),
                        'inchi' : res.get('inchi'),
                        'inchikey' : res.get('inchikey')})
