...
dot_path = {'/usr/local/graphviz/bin/dot', '/broad/software/free/Linux/redhat_5_x86_64/pkgs/graphviz_2.26.3/bin/dot'};
[status, res] = system('which dot');
dot_exec='';
if status ==0
    dot_exec = res;
else
    for ii=1:length(dot_path)
        if exist (dot_path{ii}, 'file')>0
            dot_exec = dot_path{ii};
            break
        end
    end
end
...
if ~isempty(dot_exec)
    fprintf('Dot found at:%s\n', dot_exec);
else
    fprintf('Graphviz dot not found!');
end
