...
std::string SCORE2 = "";
std::string RANK2 = "";
std::string DOWN_FILE = "offline_query_down_n250.gmt";
std::string UP_FILE = "offline_query_up_n250.gmt";
...
VS load_queries(string filename) {
  ...
  if (ext.compare(".grp") == 0) {
    string geneset;
    string nodesc = "";
    qheader.PB(filename.substr(namestart+1, extloc-(namestart+1)-3));
    qdesc.PB(nodesc);
    while (fgets(line, MAX_LINE, f)) {
      int pos = 0;
      while (line[pos] != '\n') pos++;
...
int main(int argc, char **argv){
  ...
  for (int i = 1; i < argc; ++i) {
    if (string(argv[i]) == "--calc-sig") {
      ...
      ++i;
      RANK2 = argv[i];
		}  else if (string(argv[i]) == "--num-genes") {
      ++i;
      NUM_GENES = atoi(argv[i]);
    } else if (string(argv[i]) == "--num-sigs") {
      ++i;
      NUM_SIG = atoi(argv[i]);
    } else if (string(argv[i]) == "--buffer-size") {
      ++i;
      BUFFER = atoi(argv[i]);
