...
%   'cxtpath': Path to CXT files. Default is '/cmap/obelix/bged/downloads/cxt'
%   'rid': Feautre space to include. Default is '/cmap/data/vdb/spaces/affx_n22268.grp'

pname = {'cxtpath',...
    'rid',...
    'verbose'};
dflts = {'/cmap/obelix/bged/cxt',...
    '/cmap/data/vdb/spaces/affx_n22268.grp',...
    false};
...
if ~isempty(cxt)    
    [cxtname, cxtlbl] = get_cxtname(args.cxtpath, cxt);
...
function [cxtname, cxtlbl] = get_cxtname(cxtpath, cxt)
cxtlbl = regexprep(cxt, '\.CXT\.gz$|\.gz$|\.CEL\.gz$|\.CEL$','');
cxt = strcat(cxtlbl, '.CXT.gz');
ncxt = length(cxt);
cxtname = cell(ncxt, 1);
for ii=1:ncxt
    cxtname{ii} = fullfile(cxtpath, cxt{ii});
end
end
