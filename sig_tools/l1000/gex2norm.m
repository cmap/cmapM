...
if isstruct(args.res) || isfileexist(args.res)
    ...
    %strip dimensions if present
    if ~isempty(args.rpt)
        dsname = args.rpt;
    else
        [~, dsname] = strip_dim(raw.src);
        %strip _GEX or _gex if present
        dsname = regexprep(dsname, '_gex', '', 'ignorecase');
    end
    %transform
    if ~args.islog2
    ...
    if args.save_matrix
        calibfile = fullfile(args.out, sprintf('%s_CALIB.gct',...
            dsname));
    ...
    if ~isempty(raw.cid)
        ...
        if args.save_matrix
            scfile = fullfile(args.out, sprintf('%s_NORM.gct',...
                dsname));
        ...
        if ~isempty(cidx_pass)            
            ...
            if args.save_output
                %QC stats
                sdffile = fullfile(args.out, sprintf('%s_QC_n%d.txt',...
                    dsname, nsample));
                mksdf(sdffile, sc);
                ...
                title(texify(sprintf('%s n=%d pass:%d', dsname, ...
                    nsample, nnz(qcpass))));
                h2 = subplot(2,2,2);
                ...
                title('NORM')
                if args.save_matrix
                    qnfile = fullfile(args.out, sprintf('%s_QNORM.gct',...
                        dsname));
                ...
                xlim([4 15]);
                ylim([0 75]);
                namefig(sprintf('%s_QCPLOT', dsname));
                ...
                title (texify(sprintf('%s n:%d fc:%2.1f range:%2.1f',...
                    dsname, nsample, nanmedian(fc), nanmedian(rng))))
                grid on
                ylim([0 8000])
                namefig(sprintf('%s_RANGEFC', dsname));
