...
disp('DEPRECATED. This function will be removed in the future. Use mortar.compute.Regression.testMLR instead');
...
% landmarks not used in the inference
[unusedspace, unused_ridx] = setdiff(ds.rid, union(landmarkspace, depspace));
nunused = length(unusedspace);

%reorder features [lm; unused; dep]
ds.rid = [landmarkspace; unusedspace; depspace];
% set ds.mat(1:nlm,:) = landmarks in model order
ds.mat = [ds.mat(landmarkidx,:); ds.mat(unused_ridx, :); zeros(ndep, numSamples)];
...
[gsym, desc, ezid] = ps2genesym(ds.rid);
...
if nargout==0 || isdeployed
    ...
    switch(lower(arg.outfmt))
        ...
        case 'gctx'
            ...
            mkgctx(h5_file, ds, 'matrix_class', 'double');
    end
end