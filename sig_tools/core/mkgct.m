...
check_dup_id(gex.cid, args.checkid);
check_dup_id(gex.rid, args.checkid);
...
fid = fopen(ofile,'wt');
assert(fid>=0, 'Error opening file: %s', ofile);
...
elseif isequal(args.version, 2)
    fprintf(fid,'#1.%d\n%d\t%d\n', args.version, nr, nc);
    print_dlm_line(['id'; 'Description'; gex.cid(:)],...
        'fid', fid, 'precision', args.annot_precision);
    gsidx = find(strcmp('pr_gene_symbol', gex.rhd));
    if ~isempty(gsidx)
        gex.rdesc = gex.rdesc(:, gsidx);
        gex.rhd = gex.rhd(gsidx);
    end
    nrdesc = min(1, length(gex.rhd));
else
    error('Unknown GCT version %d', args.version);
end
