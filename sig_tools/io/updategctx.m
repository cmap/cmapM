function updategctx(dsfile, ds, varargin)
...
[erid, eridx_ds, eridx_annot] = intersect(ds.rid, annot.rid);

nc = length(ecid);
nr = length(erid);
assert(isequal(nc, length(ds.cid)), 'Some column ids not found');
assert(isequal(nr, length(ds.rid)), 'Some row ids not found');

fid = H5F.open(dsfile, 'H5F_ACC_RDWR', 'H5P_DEFAULT');

%Update submatrix
update_matrix(fid, args.root, args.dsname,...
    ds.mat(eridx_ds, ecidx_ds), eridx_annot, ecidx_annot, args);
...
function update_matrix(fid, root, mid, mat, ridx, cidx, args)
%update submatrix in existing dataset
...
nc = length(cidx);
file_space_id = H5D.get_space(dset_id);
dbg(args.verbose, 'Updating %d columns', ds_dims(2));

% select submatrix
% [rnk, dims] = H5S.get_simple_extent_dims(file_space_id);
mem_space_id = H5S.create_simple(2, [nc, nr], []);
counta = [1 h5_dims(2)];
