...
if isfileexist(fname)    
    ...
    if ~isempty(expidx)    
         ...
         tok = cellfun(@(x) textscan(x, '%s\t%f\t%s'),...
                    lines(lastidx+(1:nr)), 'uniformoutput', false);
         ds.rid = cellfun(@(x) x{ridx}, tok);
         ds.mat = cellfun(@(x) x{expidx}, tok);
         ds.rdesc = cellfun(@(x) x{rhidx}, tok);
%     for ii=1:nr
%         lidx = ii + lastidx;
