...
% Write table data
if iscell(tbl)
    check_row_limit(size(tbl, 1), isxlsx)
    % cell array
    write_table(sheet, 0, 0, tbl);
elseif isstruct(tbl)    
    % struct (e.g. output of parse_tbl)
    fn = fieldnames(tbl);
    ...
    % write columns
    for ii=1:length(fn);        
        check_row_limit(size(tbl.(fn{ii}), 1)+1 , isxlsx);
...
if ~isxlsx && numrows > 65536
    error(['Rows exceed that supported by XLS [0...65535] found %d.',...
        '\nTry using XLSX instead.'], numrows)
end    
end
