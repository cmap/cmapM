...
xvals = get(event_obj.Target, 'XData');
yvals = get(event_obj.Target, 'YData');

[~, datapoint]=min(sum(bsxfun(@minus, pos', [xvals;yvals]).^2));
% now figure out which data point was selected
% datapoint = find( (xvals==pos(1))&(yvals==pos(2)) );

if isstruct(ud)
    labels = strcat(upper(fieldnames(ud)), ':', struct2cell(ud(datapoint)));
else
    labels = ud(datapoint);
end
